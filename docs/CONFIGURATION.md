...
![Synthetic preview](preview.png)

Copy `config.example.json` into `private/config.json` and put your CSV next to it as
`private/sales.csv`. Run `python -m trmnl_digital_product_sales --config private/config.json`.
Import `trmnl-digital-product-sales.zip` and add `--push` after configuring its webhook as described in the
