...
`/opt/trmnl-digital-product-sales`, including its `.venv`. Keep the code read-only to that user and give
...
```sh
sudo cp deploy/trmnl-digital-product-sales.service deploy/trmnl-digital-product-sales.timer /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now trmnl-digital-product-sales.timer
sudo systemctl start trmnl-digital-product-sales.service
sudo systemctl status trmnl-digital-product-sales.service
```
