...
New licenses, renewals and refunds from CSV. A standalone, MIT-licensed TRMNL plugin with four layouts and a
...
```sh
git clone https://github.com/JirakJ/trmnl-digital-product-sales.git
cd trmnl-digital-product-sales
python3 -m venv .venv
.venv/bin/pip install -r requirements.lock -e .
mkdir -p private
chmod 700 private
cp config.example.json private/config.json
.venv/bin/python -m trmnl_digital_product_sales --demo
```
...
```sh
.venv/bin/python -m trmnl_digital_product_sales --config private/config.json
```

1. Download `trmnl-digital-product-sales.zip` from this repository's releases.
...
The installed command `trmnl-digital-product-sales` accepts the same options. Without `--push`, only
...
`dist/trmnl-digital-product-sales.zip`; it contains only `settings.yml` and the five Liquid templates.
